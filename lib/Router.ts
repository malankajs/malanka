...
import {RouterEvent} from './declarations';

export class Router {
    protected _route:ValueProxy<string>;
    protected _lastEvent:RouterEvent;
    protected index:Object;
    protected routes:((url, query, hash, state) => RouterEvent|boolean)[];
    protected middlewares:[() => Promise<RouterEvent>, () => Promise<RouterEvent>][];

    /**
     * @param {Object} routes
     */
    constructor({routes}) {
        this._route = new ValueProxy<string>({
            get: () => {
     ...
     * @param {{}} routes
     */
    protected buildRoutes(routes:Object) {
     ...
     * @param {string} route
     */
    public addRoute(name:string, route:string):void {
        let names = [];
     ...
     */
    public match(input:string, state?):Promise<RouterEvent> {
    ...
    start() {
        ...
        document.body.addEventListener('click', (event) => {
            var link = event.target as HTMLAnchorElement;
     ...
     * @param {{}} state
     */
    navigate(url:string, {replace = false, trigger = true, state = null} = {}) {
