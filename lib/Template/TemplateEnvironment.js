...
export class TemplateEnvironment {
     ...
     */
    compile(content) {
        let compiledContent = this.scope({scope: 'context'}, () => {
            return content.compile();
        });

        let importsString = this.compileImports(this.imports);

        return `${importsString}module.exports = function(context){${this.compileTemplates()}return ${compiledContent}}`;
    }
