...
export class TemplateEnvironment {
    ...
    compile(content) {
        ...
        let hotModules = Object.keys(this.imports).map(key => {
            let {path} = this.imports[key];

            return JSON.stringify(path);
        });

        let hmr = `if (module.hot) {
        ...
        return `var ${imports.join(',\n')};\n\nmodule.exports = function(context){${hmr}; return ${compiledContent}}`;
    }
